...
__host__ void runSlice(float* resultat, int n)
    ...
    Device::lastCudaError("kernelSlice (before)"); // temp debug
    kernelSlice<<<dg,db,SIZE_SM>>>(ptrDevTotal, n, dx);
    Device::lastCudaError("kernelSlice (after)"); // temp debug
...
__host__ bool slice(int n)
    ...
    cout << " Result PI obtained : " << result << "." << endl;
